...
export default class Marital extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      showmodal: false,
    ...
    const {gender,age,hypertension,heart}= this.props.route.params;
  }
  handleButtonPress = (value3) => {
    const { gender,age,hypertension,heart } = this.state;
    this.setState({ ever_married: value3 }, () => {
      // Navigate to the next screen with the parameter value
      this.props.navigation.navigate('WorkType', { gender,age ,hypertension,heart,ever_married:value3 });
    });
  };
  render() {
    //js scope
...
<TouchableOpacity onPress={() =>this.handleButtonPress(0)}
          ...
          <TouchableOpacity onPress={() => {
              {this.props.navigation.navigate("WorkType")}}}>
            <View style={{
              width: 50,
              height: 50,
              borderRadius: 25,
